...
class ActivityStats(object):
    ...
    def __value_to_dollars(self, transaction):
        try:
            ...
            try:
                ...
                try:
                    ...
                    year = datetime.datetime.strptime(
                        transaction.find("transaction-date").get("iso-date").strip("Z"), "%Y-%m-%d"
                    ).year
                except AttributeError:
            ...
            if year == 2013:
                year = 2012
            return toUSD(value=self.__create_decimal(value.text), currency=currency, year=year)
    ...
    def spend(self):
        ...
        transactions = [
            ...
            if x.find("transaction-type") is not None and x.find("transaction-type").get("code") in ["D", "E"]
        ]
    ...
    def spend_per_organisation_type(self):
        try:
            transactions = [
                x
                for x in self.element.findall("transaction")
                if x.find("transaction-type") is not None
                and x.find("transaction-type").get("code") in ["D", "E"]
                and x.find("transaction-date") is not None
                and datetime.datetime.strptime(x.find("transaction-date").get("iso-date").strip("Z"), "%Y-%m-%d")
