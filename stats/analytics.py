...
with open("helpers/transparency_indicator/reference_spend_data.csv", "r") as csv_file:
    ...
    for line in reader:
        ...
        reference_spend_data[pub_registry_id] = {
...
def get_language(major_version, iati_activity_obj, title_or_description_obj):
    ...
    if has_xml_lang(iati_activity_obj):
        default_lang = iati_activity_obj.xpath(
            "@xml:lang", namespaces={"xml": "http://www.w3.org/XML/1998/namespace"}
        )[0]
    ...
    if major_version == "2":
        for narrative_obj in title_or_description_obj.findall("narrative"):
            if has_xml_lang(narrative_obj):
                langs.append(
                    narrative_obj.xpath("@xml:lang", namespaces={"xml": "http://www.w3.org/XML/1998/namespace"})[0]
                )
        ...
        if has_xml_lang(title_or_description_obj):
            langs.append(
                title_or_description_obj.xpath(
                    "@xml:lang", namespaces={"xml": "http://www.w3.org/XML/1998/namespace"}
                )[0]
            )
        elif has_xml_lang(iati_activity_obj):
...
class CommonSharedElements(object):
    ...
    def _participating_orgs_by_role(self):
        return dict(
            [(x.attrib.get("role"), {x.attrib.get("ref"): 1}) for x in self.element.findall("participating-org")]
        )
...
class ActivityStats(CommonSharedElements):
    ...
    def by_hierarchy(self):
        ...
        for stat in [
            ...
            "elements",
            "elements_total",
            "forwardlooking_currency_year",
            "forwardlooking_activities_current",
            "forwardlooking_activities_with_budgets",
            "forwardlooking_activities_with_budget_not_provided",
            "comprehensiveness",
    ...
    def _currencies(self):
        currencies = [
            x.find("value").get("currency") for x in self.element.findall("transaction") if x.find("value") is not None
        ]
    ...
    def __spend_currency_year(self, transactions):
        ...
        for transaction in transactions:
            ...
            if transaction.find("transaction-type") is not None and transaction.find("transaction-type").attrib.get(
                "code"
            ) in [self._disbursement_code(), self._expenditure_code()]:
    ...
    def _get_end_date(self):
        ...
        end_date_list = self.element.xpath(
            'activity-date[@type="{}"]'.format(self._actual_end_code())
        ) or self.element.xpath('activity-date[@type="{}"]'.format(self._planned_end_code()))
    ...
    def _forwardlooking_is_current(self, year):
        ...
        activity_start_years = [
            iso_date(x).year
            for x in self.element.xpath(
                'activity-date[@type="{}" or @type="{}"]'.format(self._planned_start_code(), self._actual_start_code())
            )
            if iso_date(x)
        ]
        ...
        activity_end_years = [
            iso_date(x).year
            for x in self.element.xpath(
                'activity-date[@type="{}" or @type="{}"]'.format(self._planned_end_code(), self._actual_end_code())
            )
            if iso_date(x)
        ]
        ...
        return (not activity_end_years) or (
            any(activity_end_year >= year for activity_end_year in activity_end_years)
            and any(activity_start_year <= year for activity_start_year in activity_start_years)
        )
    ...
    def _get_ratio_commitments_disbursements(self, year):
        ...
        commitment_transactions = [
            ...
            for transaction in self.element.xpath(
                'transaction[transaction-type/@code="{}"]'.format(self._commitment_code())
            )
        ]

        # Convert transaction values to USD and aggregate
        commitment_transactions_usd_total = sum(
            [get_USD_value(x[0], x[1], x[2].year) for x in commitment_transactions if None not in x]
        )
        ...
        exp_disb_transactions = [
            ...
            for transaction in self.element.xpath(
                'transaction[transaction-type/@code="{}" or transaction-type/@code="{}"]'.format(
                    self._disbursement_code(), self._expenditure_code()
                )
            )
        ]

        # If the transaction date this year or older, convert transaction values to USD and aggregate
        exp_disb_transactions_usd_total = sum(
            [
                get_USD_value(x[0], x[1], x[2].year)
                for x in exp_disb_transactions
                if None not in x and x[2].year <= int(year)
            ]
        )

        if commitment_transactions_usd_total > 0:
            return convert_to_float(exp_disb_transactions_usd_total) / convert_to_float(
                commitment_transactions_usd_total
            )
        else:
            return None
    ...
    def _forwardlooking_exclude_in_calculations(self, year=date.today().year, date_code_runs=None):
        ...
        if (
            ...
            and self._get_ratio_commitments_disbursements(year) >= 0.9
        ):
            return 2
        else:
            return 0
    ...
    def _is_donor_publisher(self):
        ...
        return (
            self.element.xpath("reporting-org/@ref")[0]
            in self.element.xpath(
                "participating-org[@role='{}']/@ref|participating-org[@role='{}']/@ref".format(
                    self._funding_code(), self._OrganisationRole_Extending_code()
                )
            )
        ) and (
            self.element.xpath("reporting-org/@ref")[0]
            not in self.element.xpath(
                "participating-org[@role='{}']/@ref".format(self._OrganisationRole_Implementing_code())
            )
        )
    ...
    def _forwardlooking_excluded_activities(self):
        ...
        try:
            return {
                self.element.find("iati-identifier").text: {
                    year: int(self._forwardlooking_exclude_in_calculations(year))
    ...
    def forwardlooking_activities_current(self, date_code_runs=None):
        ...
        return {
            year: int(
                self._forwardlooking_is_current(year)
    ...
    def forwardlooking_activities_with_budgets(self, date_code_runs=None):
        ...
        return {
            year: int(
                self._forwardlooking_is_current(year)
                and not self._budget_and_budget_not_provided()
                and year in budget_years
                and not bool(self._forwardlooking_exclude_in_calculations(year=year, date_code_runs=date_code_runs))
            )
            for year in range(this_year, this_year + 3)
        }
    ...
    def _comprehensiveness_is_current(self):
        ...
        activity_planned_end_dates = [
            iso_date(x)
            for x in self.element.xpath('activity-date[@type="{}"]'.format(self._planned_end_code()))
            if iso_date(x)
        ]
        activity_actual_end_dates = [
            iso_date(x)
            for x in self.element.xpath('activity-date[@type="{}"]'.format(self._actual_end_code()))
            if iso_date(x)
        ]
    ...
    def _is_recipient_language_used(self):
        ...
        if len(self.element.findall("recipient-country")) == 1:
            ...
            if (
                len(set(langs_in_title).intersection(country_langs)) > 0
                and len(set(langs_in_description).intersection(country_langs)) > 0
            ):
                return 1
            else:
                return 0
    ...
    def _comprehensiveness_bools(self):
        ...
        return {
            ...
            "sector": self.element.find("sector") is not None
            or (
                self._major_version() != "1"
                and all_true_and_not_empty(
                    (transaction.find("sector") is not None) for transaction in self.element.findall("transaction")
                )
            ),
            "country_or_region": (
                ...
                or (
                    ...
                    and all_true_and_not_empty(
                        (
                            transaction.find("recipient-country") is not None
                            or transaction.find("recipient-region") is not None
                        )
                        for transaction in self.element.findall("transaction")
                    )
                )
            ),
            "transaction_commitment": self.element.xpath(
                'transaction[transaction-type/@code="{}" or transaction-type/@code="11"]'.format(
                    self._commitment_code()
                )
            ),
            "transaction_spend": self.element.xpath(
                'transaction[transaction-type/@code="{}" or transaction-type/@code="{}"]'.format(
                    self._disbursement_code(), self._expenditure_code()
                )
            ),
            "transaction_currency": all_true_and_not_empty(
                x.xpath("value/@value-date") and x.xpath("../@default-currency|./value/@currency")
                for x in self.element.findall("transaction")
            ),
            "transaction_traceability": all_true_and_not_empty(
                x.xpath("provider-org/@provider-activity-id")
                for x in self.element.xpath(
                    'transaction[transaction-type/@code="{}" or transaction-type/@code="11" or transaction-type/@code="13"]'.format(
                        self._incoming_funds_code()
                    )
                )
            )
            ...
            "location": self.element.xpath(
                "location/point/pos|location/name|location/description|location/location-administrative"
            ),
            ...
            "activity-website": self.element.xpath(
                "activity-website" if self._major_version() == "1" else 'document-link[category/@code="A12"]'
            ),
            ...
            "aid_type": (
                ...
                or all_true_and_not_empty(
                    [transaction.xpath("aid-type/@code") for transaction in self.element.xpath("transaction")]
                )
            ),
    ...
    def _is_sector_dac(self):
        ...
        sector_dac_activity_level = self.element.xpath(
            'sector[@vocabulary="{}" or @vocabulary="{}" or not(@vocabulary)]'.format(
                self._dac_5_code(), self._dac_3_code()
            )
        )

        if self._major_version() != "1":
            sector_dac_transaction_level = [
                transaction.xpath(
                    'sector[@vocabulary="{}" or @vocabulary="{}" or not(@vocabulary)]'.format(
                        self._dac_5_code(), self._dac_3_code()
                    )
        ...
        def empty_or_percentage_sum_is_100(path, by_vocab=False):
            ...
            if not elements:
                ...
                if by_vocab:
                    ...
                    return (
                        len(elements) == 1 or sum(decimal_or_zero(x.attrib.get("percentage")) for x in elements) == 100
                    )

        bools = copy.copy(self._comprehensiveness_bools())
        reporting_org_ref = element_ref(self.element.find("reporting-org"))
        previous_reporting_org_refs = [
            element_ref(x) for x in self.element.xpath('other-identifier[@type="B1"]') if element_ref(x) is not None
        ]

        bools.update(
            {
                "version": bools["version"]
                and self.element.getparent().attrib["version"] in CODELISTS[self._major_version()]["Version"],
                "iati-identifier": (
                    bools["iati-identifier"]
                    and (
                        # Give v1.xx data an automatic pass on this sub condition: https://github.com/IATI/IATI-Dashboard/issues/399
                        (reporting_org_ref and self.element.find("iati-identifier").text.startswith(reporting_org_ref))
                        or any(
                            [
                                self.element.find("iati-identifier").text.startswith(x)
                ...
                "participating-org": bools["participating-org"]
                and self._funding_code() in self.element.xpath("participating-org/@role"),
                "activity-status": bools["activity-status"]
                and all_true_and_not_empty(
                    x in CODELISTS[self._major_version()]["ActivityStatus"]
                    for x in self.element.xpath("activity-status/@code")
                ),
                "activity-date": (
                    bools["activity-date"]
                    and self.element.xpath(
                        'activity-date[@type="{}" or @type="{}"]'.format(
                            self._planned_start_code(), self._actual_start_code()
                        )
                    )
                    and all_true_and_not_empty(map(valid_date, self.element.findall("activity-date")))
                ),
                "sector": (bools["sector"] and empty_or_percentage_sum_is_100("sector", by_vocab=True)),
                "country_or_region": (
                    bools["country_or_region"] and empty_or_percentage_sum_is_100("recipient-country|recipient-region")
                ),
                "transaction_commitment": (
                    bools["transaction_commitment"]
                    and all([valid_value(x.find("value")) for x in bools["transaction_commitment"]])
                    and all_true_and_not_empty(
                        any(valid_date(x) for x in t.xpath("transaction-date|value"))
                        for t in bools["transaction_commitment"]
                    )
                ),
                "transaction_spend": (
                    bools["transaction_spend"]
                    and all([valid_value(x.find("value")) for x in bools["transaction_spend"]])
                    and all_true_and_not_empty(
                        any(valid_date(x) for x in t.xpath("transaction-date|value"))
                        for t in bools["transaction_spend"]
                    )
                ),
                "transaction_currency": all(
                    all(map(valid_date, t.findall("value")))
                    and all(
                        x in CODELISTS[self._major_version()]["Currency"]
                        for x in t.xpath("../@default-currency|./value/@currency")
                    )
                    for t in self.element.findall("transaction")
                ),
                "budget": (
                    bools["budget"]
                    and not self._budget_and_budget_not_provided()
                    and all(
                        valid_date(budget.find("period-start"))
                        and valid_date(budget.find("period-end"))
                        and valid_date(budget.find("value"))
                        and valid_value(budget.find("value"))
                        for budget in bools["budget"]
                    )
                ),
                "budget_not_provided": (
                    bools["budget_not_provided"]
                    and str(self._budget_not_provided()) in CODELISTS[self._major_version()]["BudgetNotProvided"]
                    and not self._budget_and_budget_not_provided()
                ),
                ...
                "sector_dac": (
                    bools["sector_dac"]
                    and all(
                        x.attrib.get("code") in CODELISTS[self._major_version()]["Sector"]
                        for x in self.element.xpath(
                            'sector[@vocabulary="{}" or not(@vocabulary)]'.format(self._dac_5_code())
                        )
                    )
                    and all(
                        x.attrib.get("code") in CODELISTS[self._major_version()]["SectorCategory"]
                        for x in self.element.xpath('sector[@vocabulary="{}"]'.format(self._dac_3_code()))
                    )
                ),
                "document-link": all_true_and_not_empty(
                    valid_url(x)
                    and x.find("category") is not None
                    and x.find("category").attrib.get("code") in CODELISTS[self._major_version()]["DocumentCategory"]
                    for x in bools["document-link"]
                ),
                ...
                "aid_type": (
                    ...
                    (
                        all_true_and_not_empty(
                            [
                                code in CODELISTS[self._major_version()]["AidType"]
                                for code in self.element.xpath("default-aid-type/@code")
                            ]
                        )
                        ...
                        or all_true_and_not_empty(
                            [
                                ...
                                for x in [
                                    transaction.xpath("aid-type/@code")
                                    for transaction in self.element.xpath("transaction")
                                ]
                            ]
                        )
        ...
        if self._comprehensiveness_is_current():
            return {
                k: (
                    1
                    if v
                    and (k not in self.comprehensiveness_denominators() or self.comprehensiveness_denominators()[k])
                    else 0
                )
                for k, v in self._comprehensiveness_bools().items()
            }
        else:
            return {}
    ...
    def comprehensiveness_with_validation(self):
        if self._comprehensiveness_is_current():
            return {
                ...
                for k, v in self._comprehensiveness_with_validation_bools().items()
            }
        else:
            return {}
    ...
    def comprehensiveness_denominators(self):
        if self._comprehensiveness_is_current():
            dates = self.element.xpath(
                'activity-date[@type="{}"]'.format(self._actual_start_code())
            ) + self.element.xpath('activity-date[@type="{}"]'.format(self._planned_start_code()))
            if dates:
            ...
            return {
                ...
                "transaction_spend": (
                    1
                    if start_date and start_date < self.today and (self.today - start_date) > timedelta(days=365)
                    else 0
                ),
            ...
            return {"recipient_language": 0, "transaction_spend": 0, "transaction_traceability": 0}
    ...
    def humanitarian(self):
        ...
        is_humanitarian_by_attrib_activity = (
            1
            if ("humanitarian" in self.element.attrib) and (self.element.attrib["humanitarian"] in ["1", "true"])
            else 0
        )
        is_not_humanitarian_by_attrib_activity = (
            1
            if ("humanitarian" in self.element.attrib) and (self.element.attrib["humanitarian"] in ["0", "false"])
            else 0
        )
        is_humanitarian_by_attrib_transaction = (
            1 if set(self.element.xpath("transaction/@humanitarian")).intersection(["1", "true"]) else 0
        )
        is_humanitarian_by_attrib = (self._version() in ["2.02", "2.03"]) and (
            is_humanitarian_by_attrib_activity
        ...
        is_humanitarian_by_sector_5_digit_activity = (
            1
            if set(
                self.element.xpath('sector[@vocabulary="{0}" or not(@vocabulary)]/@code'.format(self._dac_5_code()))
            ).intersection(humanitarian_sectors_dac_5_digit)
            else 0
        )
        is_humanitarian_by_sector_5_digit_transaction = (
            1
            if set(
                self.element.xpath(
                    'transaction[not(@humanitarian="0" or @humanitarian="false")]/sector[@vocabulary="{0}" or not(@vocabulary)]/@code'.format(
                        self._dac_5_code()
                    )
                )
            ).intersection(humanitarian_sectors_dac_5_digit)
        ...
        is_humanitarian_by_sector_3_digit_activity = (
            1
            if set(self.element.xpath('sector[@vocabulary="{0}"]/@code'.format(self._dac_3_code()))).intersection(
                humanitarian_sectors_dac_3_digit
            )
            else 0
        )
        is_humanitarian_by_sector_3_digit_transaction = (
            1
            if set(
                self.element.xpath(
                    'transaction[not(@humanitarian="0" or @humanitarian="false")]/sector[@vocabulary="{0}"]/@code'.format(
                        self._dac_3_code()
                    )
                )
            ).intersection(humanitarian_sectors_dac_3_digit)
            else 0
        )
        # helper variables to help make logic easier to read
        is_humanitarian_by_sector_activity = (
            is_humanitarian_by_sector_5_digit_activity or is_humanitarian_by_sector_3_digit_activity
        )
        is_humanitarian_by_sector_transaction = (
            is_humanitarian_by_sector_5_digit_transaction or is_humanitarian_by_sector_3_digit_transaction
        )
        is_humanitarian_by_sector = is_humanitarian_by_sector_activity or (
            is_humanitarian_by_sector_transaction and (self._major_version() in ["2"])
        )
        ...
        return {
            ...
            "contains_humanitarian_scope": (
                ...
                if (
                    is_humanitarian
                    and self._version() in ["2.02", "2.03"]
                    and all_true_and_not_empty(self.element.xpath("humanitarian-scope/@type"))
                    and all_true_and_not_empty(self.element.xpath("humanitarian-scope/@code"))
                )
                else 0
            ),
            ...
            "uses_humanitarian_clusters_vocab": (
                1
                if (
                    is_humanitarian
                    and self._version() in ["2.02", "2.03"]
                    and self.element.xpath('sector/@vocabulary="10"')
                )
                else 0
            ),
            "uses_humanitarian_clusters_vocab_without_humanitarian": (
                1
                if (
                    (not is_humanitarian)
                    and self._version() in ["2.02", "2.03"]
            ...
            "uses_humanitarian_glide_codes": (
                1
                if (
                    is_humanitarian
                    and self._version() in ["2.02", "2.03"]
                    and self.element.xpath("humanitarian-scope/@vocabulary")
                    and self.element.xpath('humanitarian-scope/@vocabulary="1-2"')
                )
                else 0
            ),
            "uses_humanitarian_glide_codes_without_humanitarian": (
                1
                if (
                    (not is_humanitarian)
                    and self._version() in ["2.02", "2.03"]
            ...
            "uses_humanitarian_hrp_codes": (
                1
                if (
                    is_humanitarian
                    and self._version() in ["2.02", "2.03"]
                    and self.element.xpath("humanitarian-scope/@vocabulary")
                    and self.element.xpath('humanitarian-scope/@vocabulary="2-1"')
                )
                else 0
            ),
            "uses_humanitarian_hrp_codes_without_humanitarian": (
                1
                if (
                    (not is_humanitarian)
                    and self._version() in ["2.02", "2.03"]
    ...
    def _sum_transactions_by_type_by_year(self):
        ...
        for transaction in self.element.findall("transaction"):
            ...
            if transaction.find("transaction-type") is not None and transaction.find("transaction-type").attrib.get(
                ...
                self._incoming_funds_code(),
                self._commitment_code(),
                self._disbursement_code(),
                self._expenditure_code(),
            ]:
    ...
    def _participating_org_all_stats(self, org_type_id):
        ...
        return {
            "total_orgs": self._participating_org_stats(org_type_id, "total"),
    ...
    def _transaction_org_all_stats(self, org_type):
        ...
        return {
            "total_orgs": self._transaction_org_stats(org_type, "total"),
...
class GenericFileStats(object):
    ...
    def version_mismatch(self):
        ...
        return {
            (
                "true"
                if (file_version is not None and len(element_versions) and [file_version] != element_versions)
                else "false"
            ): 1
        }
...
class PublisherStats(object):
    ...
    def _reference_spend_data(self):
        ...
        if self.folder in reference_spend_data.keys():
            ...
            return {
                "2014": {
                    "ref_spend": (
                        reference_spend_data[self.folder]["2014_ref_spend"].replace(",", "")
                        if is_number(reference_spend_data[self.folder]["2014_ref_spend"].replace(",", ""))
                        else ""
                    ),
                ...
                "2015": {
                    "ref_spend": (
                        reference_spend_data[self.folder]["2015_ref_spend"].replace(",", "")
                        if is_number(reference_spend_data[self.folder]["2015_ref_spend"].replace(",", ""))
                        else ""
                    ),
                    "currency": reference_spend_data[self.folder]["currency"],
                    "official_forecast_usd": (
                        reference_spend_data[self.folder]["2015_official_forecast"].replace(",", "")
                        if is_number(reference_spend_data[self.folder]["2015_official_forecast"].replace(",", ""))
                        else ""
                    ),
                },
                "spend_data_error_reported": (
                    1 if reference_spend_data[self.folder]["spend_data_error_reported"] else 0
                ),
    ...
    def reference_spend_data_usd(self):
        ...
        for year, data in ref_data_years:
            ...
            output[year] = {}
            output[year]["ref_spend"] = (
                str(get_USD_value(data["currency"], data["ref_spend"], year)) if is_number(data["ref_spend"]) else ""
            )
            output[year]["official_forecast"] = (
                data["official_forecast_usd"] if is_number(data["official_forecast_usd"]) else ""
            )
    ...
    def date_extremes(self):
        activity_dates = {
            k: list(filter(lambda x: x is not None, map(iso_date_match, v.keys())))
        ...
        return {
            "min": {"overall": overall_min, "by_type": {k: str(v) for k, v in min_dates.items()}},
            "max": {"overall": overall_max, "by_type": {k: str(v) for k, v in max_dates.items()}},
        }

    @no_aggregation
    def most_recent_transaction_date(self):
        """Computes the latest non-future transaction data across a dataset"""
        nonfuture_transaction_dates = list(
            filter(
    ...
    def provider_activity_id_without_own(self):
        out = {
            k: v
            for k, v in self.aggregated["provider_activity_id"].items()
            if k not in self.aggregated["iati_identifiers"]
        }
        return out
    ...
    @no_aggregation
    def assessment(self):
        out = {"dimensions": {"comprehensiveness": {"components": {}}}}
        ...
        for component_number, feature_ratios in feature_ratios_by_compontent.items():
            ...
            out["dimensions"]["comprehensiveness"]["components"][component_string] = mean(feature_ratios)
...
class AllDataStats(object):
    ...
    def traceable_sum_commitments_and_disbursements_by_publisher_id(self):
        ...
        for publisher_id, d in self.aggregated[
...
class PublisherWithHistoryStats(object):
    ...
    def _timeliness(self, include_future=True):
        ...
        previous_months = [
            "{}-{}".format(year, str(month).zfill(2)) for year, month in previous_months_generator(today)
        ]
        ...
        this_month_number = self.now.month
