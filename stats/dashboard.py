...
class ActivityStats(CommonSharedElements):
    ...
    def by_hierarchy(self):
        out = {}
        for stat in [
            'activities', 'elements', 'elements_total',
    ...
    def forwardlooking_excluded_activities(self):
        ...
        return {
            self.element.find('iati-identifier').text: {
                year: int(self._forwardlooking_exclude_in_calculations(year)) for year in range(this_year, this_year + 3)
            }
        }
    ...
    def forwardlooking_activities_current(self, date_code_runs=None):
        ...
        return {
            year: int(
                self._forwardlooking_is_current(year) and not bool(self._forwardlooking_exclude_in_calculations(year=year, date_code_runs=date_code_runs))
    ...
    def forwardlooking_activities_with_budgets(self, date_code_runs=None):
        ...
        return {
            year: int(
                self._forwardlooking_is_current(year) and year in budget_years and not bool(self._forwardlooking_exclude_in_calculations(year=year, date_code_runs=date_code_runs))
            ) for year in range(this_year, this_year + 3)
        }
    ...
    def _comprehensiveness_bools(self):
        ...
        return {
            ...
            'sector': self.element.find('sector') is not None or (
            ...
            'transaction_traceability': all_true_and_not_empty(
                x.xpath('provider-org/@provider-activity-id') for x in self.element.xpath('transaction[transaction-type/@code="{}"]'.format(
            ...
            'aid_type': (
                ...
                (
                    ...
                    all_true_and_not_empty(
                        [code in CODELISTS[self._major_version()]['AidType'] for code in self.element.xpath('default-aid-type/@code')]
                    ) or all_true_and_not_empty(
                        [set(x).intersection(CODELISTS[self._major_version()]['AidType']) for x in [transaction.xpath('aid-type/@code') for transaction in self.element.xpath('transaction')]]
                    )
                )
            )
        })
    ...
    def comprehensiveness(self):
        if self._comprehensiveness_is_current():
            return {
                k: (
                    1 if v and (
                        k not in self.comprehensiveness_denominators()or self.comprehensiveness_denominators()[k]
...
class PublisherStats(object):
    ...
    def reference_spend_data(self):
        ...
        if self.folder in reference_spend_data.keys():
            ...
            return {
                '2014': {
                    'ref_spend': reference_spend_data[self.folder]['2014_ref_spend'].replace(',', '') if is_number(reference_spend_data[self.folder]['2014_ref_spend'].replace(',', '')) else '',
                    'currency': reference_spend_data[self.folder]['currency'],
                    'official_forecast_usd': ''
                },
                '2015': {
                    'ref_spend': reference_spend_data[self.folder]['2015_ref_spend'].replace(',', '') if is_number(reference_spend_data[self.folder]['2015_ref_spend'].replace(',', '')) else '',
                    'currency': reference_spend_data[self.folder]['currency'],
